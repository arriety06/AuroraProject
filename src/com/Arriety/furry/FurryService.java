...
public class FurryService {
    ...
    private static FurryService instance;

    public static FurryService gI() {
        if (instance == null) {
            instance = new FurryService();
        }
        return instance;
    }

    private List<ItemFurry> getItemKyGui(Player pl, byte tab, byte... max) {
        List<ItemFurry> its = new ArrayList<>();
        List<ItemFurry> listSort = new ArrayList<>();
        List<ItemFurry> listSort2 = new ArrayList<>();
        FurryManager.gI().listItem.stream().filter((it) -> (it != null && it.tab == tab && !it.isBuy && it.player_sell != pl.id)).forEachOrdered((it) -> {
            its.add(it);
        });
    ...
    private List<ItemFurry> getItemKyGui() {
        List<ItemFurry> its = new ArrayList<>();
        List<ItemFurry> listSort = new ArrayList<>();
        FurryManager.gI().listItem.stream().filter((it) -> (it != null && !it.isBuy)).forEachOrdered((it) -> {
            its.add(it);
        });
        its.stream().filter(i -> i != null).sorted(Comparator.comparing(i -> i.isUpTop, Comparator.reverseOrder())).forEach(i -> listSort.add(i));
        return listSort;
    ...
    public void buyItem(Player pl, int id) {
        ...
        if (isBuy) {
            ...
            if (it.isBuy) {
                ...
                Service.gI().sendThongBao(pl, "Bạn đã nhận được " + it.quantity + item.template.name);
                System.out.println(pl.name + " KyGui shop sold " + "[x" + it.quantity + item.template.name + "]");
                openShopKyGui(pl);
            }
        }
    }

    public ItemFurry getItemBuy(int id) {
        for (ItemFurry it : getItemKyGui()) {
            if (it != null && it.id == id) {
                return it;
            }
        }
    ...
    public ItemFurry getItemBuy(Player pl, int id) {
        for (ItemFurry it : FurryManager.gI().listItem) {
    ...
    public void openShopKyGui(Player pl, byte index, int page) {
        ...
        try {
            ...
            List<ItemFurry> items = getItemKyGui(pl, index);
            List<ItemFurry> itemsSend = getItemKyGui(pl, index, (byte) (page * 20), (byte) (page * 20 + 20));
            ...
            for (int j = 0; j < itemsSend.size(); j++) {
                ...
                if (pl.getSession().getVersion() >= 222) {
...
//                msg.writer().writeByte(0);
    ...
    public void claimOrDel(Player pl, byte action, int id) {
        ...
        switch (action) {
            case 1: // hủy vật phẩm
                ...
                item.quantity = it.quantity;
                item.itemOptions.addAll(it.options);
                if (FurryManager.gI().listItem.remove(it)) {
                    InventoryServiceNew.gI().addItemBag(pl, item);
                    InventoryServiceNew.gI().sendItemBags(pl);
                    Service.gI().sendMoney(pl);
    ...
    public List<ItemFurry> getItemCanKiGui(Player pl) {
        List<ItemFurry> its = new ArrayList<>();
        FurryManager.gI().listItem.stream().filter((it) -> (it != null && it.player_sell == pl.id)).forEachOrdered((it) -> {
            its.add(it);
        });
        pl.inventory.itemsBag.stream().filter((it) -> (it.isNotNullItem() && ((it.template.type == 23)))).forEachOrdered((it) -> {
            its.add(new ItemFurry(InventoryServiceNew.gI().getIndexBag(pl, it), it.template.id, (int) pl.id, (byte) 4, -1, -1, it.quantity, (byte) -1, it.itemOptions, false));
        });
        return its;
    }
    ...
    public int getMaxId() {
        try {
            ...
            FurryManager.gI().listItem.stream().filter((it) -> (it != null)).forEachOrdered((it) -> {
                id.add(it.id);
            });
    ...
    public void KiGui(Player pl, int id, int money, byte moneyType, int quantity) {
        try {
            if (!pl.getSession().getIsActive()) {
                Service.gI().sendThongBao(pl, "Bạn phải kích hoạt thành viên\n mới được đăng bán!");
                openShopKyGui(pl);
                return;
            }
            ...
            switch (moneyType) {
                case 0:// vàng
                    ...
                    FurryManager.gI().listItem.add(new ItemFurry(getMaxId() + 1, it.template.id, (int) pl.id, getTabKiGui(it), money, -1, quantity, (byte) 0, it.itemOptions, false));
                    InventoryServiceNew.gI().sendItemBags(pl);
                    openShopKyGui(pl);
                    Service.gI().sendMoney(pl);
                ...
                case 1:// hồng ngọc
                    InventoryServiceNew.gI().subQuantityItemsBag(pl, pl.inventory.itemsBag.get(id), quantity);
                    FurryManager.gI().listItem.add(new ItemFurry(getMaxId() + 1, it.template.id, (int) pl.id, getTabKiGui(it), -1, money, quantity, (byte) 0, it.itemOptions, false));
                    InventoryServiceNew.gI().sendItemBags(pl);
                    openShopKyGui(pl);
                    Service.gI().sendMoney(pl);
                    Service.gI().sendThongBao(pl, "Đăng bán thành công");
    ...
    public void openShopKyGui(Player pl) {
        ...
        try {
            ...
            for (byte i = 0; i < 5; i++) {
                if (i == 4) {
                    msg.writer().writeUTF(FurryManager.gI().tabName[i]);
                    msg.writer().writeByte(0);
                    msg.writer().writeByte(getItemCanKiGui(pl).size());
                    for (int j = 0; j < getItemCanKiGui(pl).size(); j++) {
                        ItemFurry itk = getItemCanKiGui(pl).get(j);
                        if (itk == null) {
                            continue;
                        }
                        Item it = ItemService.gI().createNewItem(itk.itemId);
                    ...
                    List<ItemFurry> items = getItemKyGui(pl, i);
                    List<ItemFurry> itemsSend = getItemKyGui(pl, i, (byte) 20);
                    msg.writer().writeUTF(FurryManager.gI().tabName[i]);
                    byte tab = (byte) (items.size() / 20 > 0 ? items.size() / 20 : 1);
                    msg.writer().writeByte(tab); // max page
                    msg.writer().writeByte(itemsSend.size());
