...
import com.girlkun.models.map.sieuhang.SieuHang;
...
public class ChangeMapService {
    ...
    public void openZoneUI(Player pl) {
        if (pl.haveDuongTang) {
            Service.getInstance().sendThongBao(pl, "Đang dắt đường tăng khôg thể đổi khu");
            return;
        }
        if (pl.zone == null) {
            ...
            for (Zone zone : pl.zone.map.zones) {
                ...
                if (pl.zone.map.mapId == 113) {
                    ...
                    for (SieuHang sieuHang : SieuHangManager.gI().list) {
                        if (sieuHang.getPlayer().zone.zoneId == zone.zoneId) {
                            namerank1 += sieuHang.getPlayer().name;
                            rank1 += (int) sieuHang.getPlayer().rankSieuHang;
                            namerank2 = sieuHang.getBoss().name;
                            rank2 += (int) sieuHang.getBoss().rankSieuHang;
                            timduoc = true;
                        }
                    }
                    if (timduoc) {
                        msg.writer().writeByte(1);
                        msg.writer().writeUTF(namerank1);
                        msg.writer().writeInt(rank1);
                        msg.writer().writeUTF(namerank2);
                        msg.writer().writeInt(rank2);
                    } else {
    ...
    public void changeZone(Player pl, int zoneId) {
        ...
        if (pl.isAdmin() || pl.isBoss || Util.canDoWithTime(pl.iDMark.getLastTimeChangeZone(), 10000)) {
            ...
            if (zoneId >= 0 && zoneId <= map.zones.size() - 1) {
                ...
                if (zoneJoin != null) {
                    if (pl.idNRNM != -1 && !Util.canDoWithTime(pl.lastTimePickNRNM, 15000)) {
                        Service.gI().sendThongBao(pl, "Ngọc rồng namec quá nặng vui lòng đợi một chút để chuyển khu");
                        return;
                    }
                    changeMap(pl, zoneJoin, -1, -1, pl.location.x, pl.location.y, NON_SPACE_SHIP);
                    if (pl.idNRNM != -1) {
                        pl.lastTimePickNRNM = System.currentTimeMillis();
                        NgocRongNamecService.gI().mapNrNamec[pl.idNRNM - 353] = pl.zone.map.mapId;
                        NgocRongNamecService.gI().zoneNrNamec[pl.idNRNM - 353] = (byte) pl.zone.zoneId;
                        if (NgocRongNamecService.gI().canCallDragonNamec(pl)) {
                            NgocRongNamecService.gI().timedu7v = System.currentTimeMillis();
                        }
                    }
                }
            } else {
    ...
    private void changeMap(Player pl, Zone zoneJoin, int mapId, int zoneId, int x, int y, byte typeSpace) {
        ...
        if (zoneJoin != null) {
            ...
            if (pl != null && pl.zone != null && !pl.isBoss && !pl.isPet && !pl.isNewPet) {
    ...
    public void changeMapWaypoint(Player player) {
        if (player != null && player.zone != null) {
            ...
            if (zoneJoin != null) {
                if (player.idNRNM != -1 && !Util.canDoWithTime(player.lastTimePickNRNM, 15000)) {
                    resetPoint(player);
    ...
    private void sendEffectMapToMe(Player player) {
        ...
        try {
            for (Mob mob : player.zone.mobs) {
                if (player == null || player.effectSkill == null || mob == null || mob.isDie() || player.isDie()) {
                    continue;
                }
...
//            return null;
//        }
        if (player.isPet || player.isBoss || player.isNewPet || player.getSession() != null && player.isAdmin()) {
            return zoneJoin;
        }
    ...
    private void checkJoinSpecialMap(Player player) {
        if (player != null && player.zone != null) {
            switch (player.zone.map.mapId) {
                ...
                case 90:
                case 91:
                    BlackBallWar.gI().setflagjoinNRSD(player);
                    break;
            }
        }
    }
    ...
    private void checkJoinMapMaBu(Player player) {
        if (player != null && player.zone != null) {
            switch (player.zone.map.mapId) {
                ...
                case 119:
                case 120:
                    MapMaBu.gI().setflagjoinMabu(player);
                    break;
            }
        }
    }
