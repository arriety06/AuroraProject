...
public class UseItem {
    ...
    public void getItem(ISession session, Message msg) {
...
//            System.out.println(type + " " + where + " " + index);
            ...
            switch (type) {
                ...
                case DO_THROW_ITEM:
                    if (!(player.zone.map.mapId == 21 || player.zone.map.mapId == 22 || player.zone.map.mapId == 23)) {
                        ...
                        msg.writer().writeByte(index);
                        if (item.isNotNullItem()) {
                            if ((item.template.type >= 0 && item.template.type <= 4) || (item.template.id >= 222 && item.template.id <= 226)) {
                                if (!item.isSKHVip() && !item.isSKHThuong() && !item.isDHD() && !item.isDTL() && !item.isDTS()) {
                                    msg.writer().writeUTF("Bạn chắc chắn muốn vứt " + item.template.name + "?");
                                } else {
                                    msg.writer().writeUTF("Bạn chắc chắn muốn vứt " + item.template.name + "mất vĩnh viễn?");
                                }
                            } else {
                                msg.writer().writeUTF("Bạn chắc chắn muốn vứt " + item.template.name + "mất vĩnh viễn?");
                            }
    ...
    private void useItem(Player pl, Item item, int indexBag) {
        if (item != null && item.template != null && pl != null && pl.nPoint != null) {
                ...
                switch (item.template.type) {
                    ...
                    default:
                        switch (item.template.id) {
                            case 460:
                                SoiHecQuyn.PlayerUseCucXuong(pl, item, item.template.id);
                                break;
                            case 992:
                                pl.type = 1;
                                pl.maxTime = 5;
                            ...
                            case 361:
                                ...
                                NpcService.gI().createMenuConMeo(pl, ConstNpc.CONFIRM_TELE_NAMEC, -1, "1 Sao (" + NgocRongNamecService.gI().getNameMap(pl, 0, (short) 353) + " " + NgocRongNamecService.gI().getDis(pl, 0, (short) 353) + " m)\n2 Sao (" + NgocRongNamecService.gI().getNameMap(pl, 1, (short) 354) + " " + NgocRongNamecService.gI().getDis(pl, 1, (short) 354) + " m)\n3 Sao (" + NgocRongNamecService.gI().getNameMap(pl, 2, (short) 355) + " " + NgocRongNamecService.gI().getDis(pl, 2, (short) 355) + " m)\n4 Sao (" + NgocRongNamecService.gI().getNameMap(pl, 3, (short) 356) + " " + NgocRongNamecService.gI().getDis(pl, 3, (short) 356) + " m)\n5 Sao (" + NgocRongNamecService.gI().getNameMap(pl, 4, (short) 357) + " " + NgocRongNamecService.gI().getDis(pl, 4, (short) 357) + " m)\n6 Sao (" + NgocRongNamecService.gI().getNameMap(pl, 5, (short) 358) + " " + NgocRongNamecService.gI().getDis(pl, 5, (short) 358) + " m)\n7 Sao (" + NgocRongNamecService.gI().getNameMap(pl, 6, (short) 359) + " " + NgocRongNamecService.gI().getDis(pl, 6, (short) 359) + " m)", "Đến ngay\nViên " + (pl.idGo + 1) + " Sao\n50 ngọc", "Kết thức");
...
//    }
    ...
    private void openWoodChest(Player pl, Item item) {
        ...
        if (time != 0) {
            ...
            String text = "Bạn nhận được\n";
            InventoryServiceNew.gI().subQuantityItemsBag(pl, item, 1);
            Item itemmanh = ItemService.gI().createNewItem((short) 1108);
            int sl = 1 * param;
            itemmanh.quantity = sl;
            InventoryServiceNew.gI().addItemBag(pl, itemmanh);
            text += "x" + sl + " " + itemmanh.template.name;
    ...
    private void openCapsuleUI(Player pl) {
        if (pl.haveDuongTang) {
            Service.getInstance().sendThongBao(pl, "Đang dắt đường tăng khôg thể sài causun");
            return;
        }
    ...
    public void showlistboss(Player player, Item item) {
        InventoryServiceNew.gI().subQuantityItemsBag(player, InventoryServiceNew.gI().findItemBag(player, 1117), 1);
        BossManager.gI().showListBoss(player);
    }

    public void CreateMabu(Player player, Item item) {
        InventoryServiceNew.gI().subQuantityItemsBag(player, InventoryServiceNew.gI().findItemBag(player, 1124), 1);
        MabuEgg.createMabuEgg(player);
    }

    public void choseMapCapsule(Player pl, int index) {
        if (index < 0 || index >= pl.mapCapsule.size()) {
            Service.gI().sendThongBao(pl, "Lỗi!");
            return;
        }
        int zoneId = -1;
    ...
    public void eatPea(Player player) {
        ...
        if (pea != null) {
            ...
            if (player.pet != null && player != null && player.zone != null && player.zone.equals(player.pet.zone) && !player.pet.isDie()) {
