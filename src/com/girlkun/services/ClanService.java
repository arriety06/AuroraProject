            ...
            switch (type) {
                case CHAT:
                    ...
                    try {
                        ...
                        chatMessage = new DataInputStream(new ByteArrayInputStream(data)).readUTF();
    ...
    public void closes() {
        ...
        try (Connection con = GirlkunDB.getConnection();) {
            ...
            for (Clan clan : Manager.CLANS) {
                ...
                String bando = "[";
                bando += clan.banDoKhoBau_lastTimeOpen + ",";
                bando += "\"" + clan.banDoKhoBau_playerOpen + "\"]";
                ps.setString(10, bando);
    ...
    public void close() {
        for (Clan clan : Manager.CLANS) {
            ...
            String query = " update clan_sv" + Manager.SERVER
                    + " set slogan = ?, img_id = ?, power_point = ?, max_member = ?, clan_point = ?,"
                    + "level = ?, members = ? ,doanh_trai = ?,ban_do_kho_bau = ?  where id = ?";
            try {
