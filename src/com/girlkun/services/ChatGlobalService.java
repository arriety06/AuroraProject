...
public class ChatGlobalService implements Runnable {
    ...
    public void chat(Player player, String text) {
        try {
        if (player != null) {
            if (Setting.LOG_CHAT_GLOBAL) {
                if (!player.isAdmin()) {
                    Service.getInstance().sendThongBao(player, "|5|CHAT THẾ GIỚI TẠM ĐÓNG!");
                    return;
                }
            }
            ...
            if (player.inventory.gem >= 5) {
                if (player.isAdmin() || Util.canDoWithTime(player.iDMark.getLastTimeChatGlobal(), 360000)) {
                    if (player.isAdmin() || player.nPoint.power > 2000000000) {
                        ...
                        Service.getInstance().sendThongBaoFromAdmin(player, "Sức mạnh phải ít nhất 2 tỷ mới có thể chat thế giới");
                    }
                } else {
                    Service.getInstance().sendThongBao(player, "Không thể chat thế giới lúc này, vui lòng đợi "
                            + TimeUtil.getTimeLeft(player.iDMark.getLastTimeChatGlobal(), 360000));
                }
            } else {
                Service.getInstance().sendThongBaoFromAdmin(player, "Không đủ ngọc hồng để chat thế giới");
            }
        }
        } catch (Exception e) {
            e.printStackTrace();
