...
public class PlayerService {
    ...
    public void sendInfoHpMpMoney(Player player) {
        ...
        try {
            ...
            try {
                if (player.getSession() != null && player.getSession().getVersion() >= 214) {
    ...
    public void banPlayer(Player playerBaned) {
        try {
            GirlkunDB.executeUpdate("update account set ban = 1 where id = ? and username = ?",
                    playerBaned.getSession().getUserId(), playerBaned.getSession().getUserName());
        } catch (Exception e) {
            e.printStackTrace();
        }
