...
//                    break;
    ...
    public void checkDoneTaskKillBoss(Player player, Boss boss) {
        if (player != null && !player.isBoss && !player.isPet && !player.isNewPet) {
            switch ((int) boss.id) {
                case BossID.KUKU:
...
//        }
    ...
    private boolean isCurrentTask(Player player, int idTaskCustom) {
        ...
        try {
            if (player != null
                ...
                result = idTaskCustom == (player.playerTask.taskMain.id << 10) + player.playerTask.taskMain.index << 1;
            }
        } catch (Exception e) {
            e.printStackTrace();
        }
    ...
    public int getIdTask(Player player) {
        if (player == null || player.isPet || player.isNewPet || player.isBoss || player.playerTask == null || player.playerTask.taskMain == null) {
            return -1;
        }
    ...
    public void changeSideTask(Player player, byte level) {
        if (player != null
                ...
                && player.playerTask != null
                && player.playerTask.sideTask != null
                && player.playerTask.sideTask.leftTask > 0) {
