...
import com.Arriety.newSkillHandler.newSkillHandler;
...
public class SkillService {
    ...
    public boolean bienskill(Player player, Player plTarget, Mob mobTarget) {
...
//                Service.getInstance().releaseCooldownSkill(player);
            ...
            case 4:
                if (player.isPl()) {
                    if (player.getSession().getVersion() >= 231) {
    ...
    public void updateSkillSpecial(Player player) {
        try {
            if (player != null
                    && player.skillSpecial != null
                    && player.effectSkill != null
                    && player.skillSpecial.skillSpecial.template != null) {
        ...
        try {
            ...
            for (Player playerMap : player.skillSpecial.playersTaget) {
                new Thread(() -> {
                    try {
                        Thread.sleep(2800);
                        playerMap.effectSkill.isMaPhong3 = true;
                        playerMap.effectSkill.TimeMaPhong3 = 12000;
                        playerMap.effectSkill.lastTimeMaPhong3 = System.currentTimeMillis();
            ...
            for (Mob mobMap : player.skillSpecial.mobsTaget) {
                new Thread(() -> {
                    try {
                        Thread.sleep(2800);
                        mobMap.effectSkill.isMaPhong3 = true;
                        mobMap.effectSkill.timeMaPhong3 = 100000;
                        mobMap.effectSkill.lastTimeMaPhong3 = System.currentTimeMillis();
                        Service.gI().ChangeMonsterBody(player, 1, mobMap.id, 11166);
    ...
    private void useSkillAttack(Player player, Player plTarget, Mob mobTarget) {
        if (!player.isBoss && player != null && plTarget != null && mobTarget != null) {
            if (player.isPet) {
...
//                    mobTarget.attackMob(player, false, false);
        ...
        if (player != null && !player.isBoss && player.effectSkin != null) {
    ...
    public void useSkillAlone(Player player) {
        ...
        switch (player.playerSkill.skillSelect.template.id) {
            ...
            case Skill.HUYT_SAO:
                ...
                if (player == null || player.playerSkill == null || player.playerSkill.skillSelect == null || player.nPoint == null) {
                    return;
                }
                if (player.zone != null) {
                    if (!MapService.gI().isMapOffline(player.zone.map.mapId)) {
                        ...
                        for (Player pl : playersMap) {
                            if (pl != null) {
                                ...
                                if (!pl.isNewPet && !pl.isBoss && pl.gender != ConstPlayer.NAMEC
...
//                                    System.out.println("okok");
            ...
            case Skill.TRI_THUONG:
                ...
                if (canHsPlayer(player, plTarget)) {
                    ...
                    for (Player pl : players) {
                        ...
                        int hpHoi = 0;
                        int mpHoi = 0;
                        hpHoi += ((long) pl.nPoint.hpMax * percentTriThuong / 100);
                        mpHoi += ((long) pl.nPoint.mpMax * percentTriThuong / 100);
                    ...
                    int hpHoiMe = 0;
                    hpHoiMe += ((long) player.nPoint.hp * percentTriThuong / 100);
    ...
    private void phanSatThuong(Player plAtt, Player plTarget, int dame) {
        ...
        if (percentPST != 0) {
            int damePST = 0;
            damePST += ((long) dame * percentPST / 100);
            Message msg;
            try {
                msg = new Message(56);
    ...
    public void playerAttackPlayer(Player plAtt, Player plInjure, boolean miss) {
        if (plAtt == null || plInjure == null || plAtt.playerSkill == null) {
            return;
        }
...
//            msg.writer().writeShort(player.playerSkill.skillSelect.skillId);
    ...
    private void affterUseSkill(Player player, int skillId) {
        if (player != null && player.playerIntrinsic.intrinsic != null) {
            if (player.isPl() && player.playerIntrinsic != null) {
    ...
    private boolean canAttackPlayer(Player p1, Player p2) {
        ...
        if ((p1.zone.map.mapId == 129 || p1.zone.map.mapId == 113) && (p1.typePk > 0 && p2.typePk > 0)) {
            return true;
        }
