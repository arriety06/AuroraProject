...
import com.Arriety.furry.ItemFurry;
...
public class Manager {
    ...
    public static int MAX_PLAYER = 100000000;
    public static int RATE_EXP_SERVER = 2;
    public static boolean LOCAL = false;
    public static final int MAX_POOL = 5000000;
//    public static byte RATE_EXP_SERVER = 1;// sau khi chinh

    private void __________________List_Array_____________________________() {
        //**********************************************************************
    }
    ...
    public static final String queryTopNap = "SELECT player.id, thoi_vang FROM player INNER JOIN account ON account.id = player.account_id WHERE thoi_vang > 0 ORDER BY thoi_vang DESC LIMIT 10;";
    public static final String topsmdetu = "SELECT id, CAST(SUBSTRING_INDEX(SUBSTRING_INDEX(pet, ',', 8), ',', -1) AS UNSIGNED) AS top_sm\n"
            + "FROM player\n"
            + "ORDER BY top_sm DESC\n"
            + "LIMIT 10;";
    ...
    public static List<TOP> datatopSieuHang;
    ...
    public static List<TOP> realTopSieuHang(Player pl) {
        ...
        try {
            int ranksh = (int) pl.rankSieuHang;
            if (ranksh <= 10) {
                ranksh = 10;
            }
            GirlkunResultSet rs = GirlkunDB.executeQuery("SELECT id, CAST(split_str(data_point, ',', 15) AS UNSIGNED) AS rank FROM player WHERE CAST(split_str(data_point, ',', 15) AS UNSIGNED) <= " + ranksh + " AND CAST(split_str(data_point, ',', 15) AS UNSIGNED) >= " + (ranksh - 9) + " ORDER BY CAST(split_str(data_point, ',', 15) AS UNSIGNED) ASC LIMIT 10");
            while (rs.next()) {
    ...
    public static List<TOP> realTopSieuHang(Connection con) {
        List<TOP> tops = new ArrayList<>();
        try {
            PreparedStatement ps = con.prepareStatement("SELECT id, CAST( split_str(data_point,',',15) AS UNSIGNED) AS rank FROM player ORDER BY CAST( split_str(data_point,',',15) AS UNSIGNED) ASC LIMIT 100");
    ...
    public static List<TOP> realDataTopSieuHang(Connection con) {
        List<TOP> tops = new ArrayList<>();
        try {
            PreparedStatement ps = con.prepareStatement("SELECT id, CAST( split_str(data_point,',',15) AS UNSIGNED) AS rank FROM player WHERE CAST( split_str(data_point,',',15)  AS UNSIGNED) > 0 ORDER BY CAST( split_str(data_point,',',15)  AS UNSIGNED) ASC LIMIT 1000;");
            ResultSet rs = ps.executeQuery();
            while (rs.next()) {
                long rank = rs.getLong("rank");
                if (rank > 0 && rank <= 1000) {
    ...
    private void initMap() {
        ...
        for (MapTemplate mapTemp : MAP_TEMPLATES) {
            ...
            if (tileMap != null) {
                com.girlkun.models.map.Map map = new com.girlkun.models.map.Map(
                ...
                System.out.println("Lỗi map: " + mapTemp.name);
            }
        }
...
//                    dataArray.clear();
            ...
            Logger.log(Logger.YELLOW_BOLD_BRIGHT, "Finish load item Furry[" + FurryManager.gI().listItem.size() + "]!\n");
            ps = con.prepareStatement("SELECT * FROM shop_furry");
            rs = ps.executeQuery();
            while (rs.next()) {
                int i = rs.getInt("id");
                int idPl = rs.getInt("player_id");
                ...
                FurryManager.gI().listItem.add(new ItemFurry(i, itemId, idPl, tab, gold, gem, quantity, isUp, op, isBuy));
            }
            Logger.log(Logger.YELLOW_BOLD_BRIGHT, "Finish load item Furry [" + FurryManager.gI().listItem.size() + "]!\n");
            ...
            Logger.success("Load top sm thành công (" + Util.numberToMoney(topSM.size()) + ")\n");
            topNV = realTop(queryTopNV, con);
            Logger.success("Load top nv thành công (" + Util.numberToMoney(topNV.size()) + ")\n");
            topSK = realTop(queryTopSK, con);
            Logger.success("Load top sk thành công (" + Util.numberToMoney(topSK.size()) + ")\n");
            topPVP = realTop(queryTopPVP, con);
            Logger.success("Load top pvp thành công (" + Util.numberToMoney(topSK.size()) + ")\n");
            topNap = realTop(queryTopNap, con);
            Logger.success("Load top KhiGas thành công (" + Util.numberToMoney(topNap.size()) + ")\n");
            ...
            while (rs.next()) {
                ...
                switch (query) {
                    case queryTopSM:
                        ...
                        top.setInfo2(Util.numberToMoney(rs.getLong("sm")) + "");
                        break;
                    case queryTopNV:
                        ...
                        break;
                    case queryTopSK:
                        top.setInfo1(Util.numberToMoney(rs.getLong("event")) + " điểm");
                        top.setInfo2(Util.numberToMoney(rs.getLong("event")) + " điểm");
                        break;
                    case queryTopPVP:
                        top.setInfo1(Util.numberToMoney(rs.getLong("pointPvp")) + " điểm");
                        top.setInfo2(Util.numberToMoney(rs.getLong("pointPvp")) + " điểm");
                        break;
                    case queryTopNap:
                        top.setInfo1(Util.numberToMoney(rs.getLong("thoi_vang")) + " đồng");
                        top.setInfo2(Util.numberToMoney(rs.getLong("thoi_vang")) + " đồng");
                        break;
                }
                tops.add(top);
            }
