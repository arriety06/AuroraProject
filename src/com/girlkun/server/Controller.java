...
public class Controller implements IMessageHandler {
    ...
    @Override
    public void onMessage(ISession _session, Message _msg) {
//        System.out.println("NghiaNV Controller onMessage: " + _msg.command);
                ...
                case -100:
                    ...
                    switch (action) {
                        case 0:
                            ...
                            int quantity;
                            if (player.getSession().getVersion() >= 222) {
...
//                    break;
                ...
                case 112:
                    ...
                    break;
                case -34:
                    if (player != null && player.magicTree != null) {
...
//                        }
                ...
                case -71:
                    try {
                        String chatText = _msg.reader().readUTF();
                        ChatGlobalService.gI().chat(player, chatText);
                    } catch (IOException e) {
                        e.printStackTrace();
...
//                    System.out.println("ReQuest: " + modId);
                    break;
                case 44:
                    if (player != null) {
                ...
                case -20:
                    if (player != null && !player.isDie()) {
                        short itemMapId = _msg.reader().readShort();
...
//                System.out.println("CMD receive -28 / " + cmd);
    ...
    public void createChar(ISession session, Message msg) {
        if (!Maintenance.isRuning) {
            ...
            try {
                if (msg.reader().available() < 3) {
                    Service.getInstance().sendThongBaoOK(session, "Dữ liệu không hợp lệ");
                    return;
                }
                ...
                if (name.length() <= 10) {
                    ...
                    if (rs.first()) {
                        ...
                        if (Util.haveSpecialCharacter(name)) {
                            ...
                            if (isNotIgnoreName) {
                                created = PlayerDAO.createNewPlayer(session.getUserId(), name.toLowerCase(), (byte) gender, hair);
                            }
                        }
                    }
                } else {
                ...
                Service.getInstance().sendThongBaoOK(session, "Lỗi đọc dữ liệu");
                Logger.logException(Controller.class, e);
            } catch (Exception e) {
                Service.getInstance().sendThongBaoOK(session, "Lỗi xử lý dữ liệu");
                Logger.logException(Controller.class, e);
            } finally {
                if (rs != null) {
                    rs.dispose();
            ...
            if (created) {
                session.login(session.getUserName(), session.getPassword());
            }
        }
    }

    public void login2(ISession session, Message msg) {
//        Service.getInstance().switchToRegisterScr(session);
    ...
    public void sendInfo(ISession session) {
        ...
        if (player != null && player.inventory != null) {
            Service.getInstance().sendMessageNangDong(session, player.inventory.pointNangDong);
        }
        // -107 have pet
        Service.getInstance().sendHavePet(player);

        // -119 top rank
        Service.getInstance().sendMessageRanksh(session, (int) player.rankSieuHang);
    ...
    private void sendThongBaoServer(Player player) {
        if (player != null) {
            Service.getInstance().sendThongBaoFromAdmin(player, "Chúc bạn chơi game vui vẻ...");
        }
    }
