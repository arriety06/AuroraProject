...
public class Clan {
    ...
    public void addSMTNClan(Player plOri, long param) {
        if (this.membersInGame == null || plOri == null || plOri.zone == null) {
            return;
        }
    ...
    public void sendMessageClan(ClanMessage cmg) {
        ...
        try {
            ...
            for (int i = 0; i < membersInGame.size(); i++) {
                Player pl = membersInGame.get(i);
                pl.sendMessage(msg);
            }
            msg.cleanup();
        } catch (Exception e) {
    ...
    public void insert() {
        ...
        try (Connection con = GirlkunDB.getConnection();) {
            ps = con.prepareStatement("insert into clan_sv" + Manager.SERVER
                    + "(id, name, slogan, img_id, power_point, max_member, clan_point, level, members, doanh_trai, ban_do_kho_bau) "
                    + "values (?,?,?,?,?,?,?,?,?,?,?)");
            ps.setInt(1, this.id);
            ...
            ps.setString(10, "[" + System.currentTimeMillis() + ",\"Arriety\"]");
            ps.setString(11, "[" + System.currentTimeMillis() + ",\"Arriety\"]");
            ps.executeUpdate();
            ps.close();
        } catch (Exception e) {
            e.printStackTrace();
    ...
    public void updatePlClan(int id) {
        PreparedStatement ps;
        try (Connection con = GirlkunDB.getConnection()) {
            ...
            e.printStackTrace();
            Logger.logException(Clan.class, e, "Có lỗi khi cập nhật player");
        }
    }
}
