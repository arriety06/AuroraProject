...
public class ShopServiceNew {
    ...
    private boolean subMoneyByItemShop(Player player, ItemShop is) {
        ...
        if (is.tabShop.shop.tagName.equals("BILL")) {
            boolean iscothucan = false;
            for (Item i : player.inventory.itemsBag) {
                if (i.template != null) {
                    if (ItemData.list_thuc_an.contains((int) i.template.id)) {
                        if (InventoryServiceNew.gI().findItemBag(player, i.template.id).quantity >= 99) {
                            InventoryServiceNew.gI().subQuantityItemsBag(player, InventoryServiceNew.gI().findItemBag(player, i.template.id), 99);
                            iscothucan = true;
                            break;
                        }
                    }
            ...
            if (!iscothucan) {
                Service.getInstance().sendThongBao(player, "Không tìm thấy thức ăn");
                return false;
            }
        }
    ...
    public void buyItem(Player player, int itemTempId) {
        ...
        if (itemTempId == 711) {//chun
            boolean hasRequiredItem = false;
            Item requiredItem = InventoryServiceNew.gI().findItemBag(player, 710);// quy lao
    ...
    public void showConfirmSellItem(Player pl, int where, int index) {
        ...
        if (where == 0) {
            ...
            if (pl.getSession().getVersion() < 220) {
    ...
    private void getItemSideBoxLuckyRound(Player player, List<Item> items, byte type, int index) {
        ...
        // Tạo một danh sách mới để lưu các phần tử cần xóa
            ...
            case 0: // Nhận
                if (item.isNotNullItem()) {
                    if (InventoryServiceNew.gI().getCountEmptyBag(player) != 0) {
                        ...
                        InventoryServiceNew.gI().sendItemBags(player);
                        itemsToRemove.add(item); // Thêm item vào danh sách cần xóa
                    } else {
                        Service.getInstance().sendThongBao(player, "Hành trang đã đầy");
                    }
                } else {
        ...
        // Xóa các phần tử khỏi danh sách gốc
