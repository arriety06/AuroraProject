...
public class TrungUyXanhLoBdkb extends Boss {
    ...
    public TrungUyXanhLoBdkb(int dame, int hp, Zone zone) throws Exception {
        super(BossID.TRUNG_UY_XANH_LO_BDKB, TRUNG_UY_XANH_LO);
        this.dameClan = dame;
        this.hpClan = hp;
        this.zoneFinal = zone;
    }
...
//            ChangeMapService.gI().changeMap(this, zoneFinal, 147, 456);
    ...
    public Player getPlayerAttack() {
        try {
            ...
            if (this.playerTarger != null && this.typePk == ConstPlayer.NON_PK) {
                if (this.playerTarger != null && this.playerTarger.location != null && (this.playerTarger.location.x > -50 || this.playerTarger.location.x < 500) && this.playerTarger.location.y < 300) {
                    this.playerTarger = null;
                    this.lastTimeTargetPlayer = System.currentTimeMillis();
                    this.timeTargetPlayer = Util.nextInt(5000, 7000);
                }
                if (this.playerTarger != null && this.playerTarger.location != null && this.playerTarger.location.y > 300 && this.playerTarger.location.x > 500) {
                    this.playerTarger = null;
                    this.lastTimeTargetPlayer = System.currentTimeMillis();
                    this.timeTargetPlayer = Util.nextInt(5000, 7000);
                }
            ...
            e.printStackTrace();
            System.out.println("Lỗi ở classs TUXL getpkat");
        }
        ...
        if (Util.canDoWithTime(this.lastTimeAttack, 100) && this.typePk == ConstPlayer.PK_ALL) {
            ...
            try {
                ...
                if (Util.getDistance(this, pl) <= 100) {
                    ...
                    SkillService.gI().bienskill(this, pl, null);
                    checkPlayerDie(pl);
                } else {
    ...
    public void reward(Player plKill) {
        ...
        plKill.clan.banDoKhoBau.lasttimepick = System.currentTimeMillis();
//        plKill.clan.banDoKhoBau.setLastTimeOpen(System.currentTimeMillis() + 30000);
        int lvbdkb = plKill.clan.banDoKhoBau.level;
        int sltvdrop = (lvbdkb / 10 + 1);
        for (int i = 0; i < sltvdrop; i++) {
            Service.gI().dropItemMap(this.zone, new ItemMap(zone, 457, 1, Util.nextInt(-5, 5) * i + this.location.x, zone.map.yPhysicInTop(this.location.x, this.location.y - 24), -1));
        }
