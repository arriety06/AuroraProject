...
public class Chaien extends Boss {
    ...
    public void reward(Player plKill) {
        int[] itemDos = new int[]{555, 557, 559, 556, 558, 560, 674, 564, 566, 563, 565, 567};
        int[] NRs = new int[]{1061, 16};
        int randomDo = new Random().nextInt(itemDos.length);
        int randomNR = new Random().nextInt(NRs.length);
        if (Util.isTrue(1, 5)) {
            if (Util.isTrue(15, 100)) {
                Service.getInstance().dropItemMap(this.zone, Util.ratiItem(zone, 674, 1, this.location.x, this.location.y, plKill.id));
                return;
            }
