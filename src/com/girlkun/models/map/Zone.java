...
public class Zone {
    ...
    public short countItemAppeaerd = 0;
    ...
    private final List<Player> newpets;
    ...
    public boolean finishmappvp;// xac dinh map pvp
    ...
    private void udMob() {
        try {
            ...
            e.printStackTrace();
            System.out.println("Lỗi udMob() ở zone");
        }
    }

    private void udPlayer() {
        try {
            if (this.players != null && this.notBosses != null) {
                for (int i = this.notBosses.size() - 1; i >= 0; i--) {
                    if (i < 0 || i >= this.notBosses.size()) {
                        continue;
                    }
    ...
    private void udItem() {
        try {
            if (!this.items.isEmpty()) {
                List<ItemMap> itemMs = items;
                if (!itemMs.isEmpty()) {
                    for (int i = 0; i < itemMs.size(); i++) {
                        ItemMap itemM = itemMs.get(i);
                        if (itemM != null && itemM.zone != null) {
                            itemM.update();
                        }
                    }
                }
            ...
            e.printStackTrace();
            System.out.println("Lỗi ở udItem()");
        }
    }
    ...
    public int getnumnewpet() {
        return this.newpets.size();
    }
        ...
        for (ItemMap item : items) {
            if (item != null && item.itemTemplate != null) {
                ...
                if (item.itemTemplate.id == 78) {
                    if (taskId != ConstTask.TASK_3_1) {
                        continue;
                    }
                }
                if (item.itemTemplate.id == 74) {
                    if (taskId < ConstTask.TASK_3_0) {
                        continue;
                    }
                }
                list.add(item);
...
//    }
    ...
    public void pickItem(Player player, short itemMapId) {
        ...
        try {
            if (player != null && itemMap != null) {
                if (itemMap.playerId == player.id || itemMap.playerId == -1) {
                    ...
                    boolean picked = true;
                    if (!ItemMapService.gI().isNamecBall(item.template.id) && item.template.id != 362 && item.template.type != 22) {
                        picked = InventoryServiceNew.gI().addItemBag(player, item);
                    } else {
                        if (ItemMapService.gI().isNamecBall(item.template.id) && player.idNRNM == -1) {
                            picked = true;
                        } else {
                            picked = false;
                        }
                    ...
                    if (picked) {
                        ...
                        String text = "";
                        if (ItemMapService.gI().isNamecBall(item.template.id) && player.idNRNM != -1) {
                            text = "Bạn đã mang ngọc rồng trên người";
                            Service.gI().sendThongBao(player, text);
                        } else if (item.template.id == 362) {
                            text = "Chỉ là cục đá thôi, nhặt làm gì?";
                            Service.gI().sendThongBao(player, text);
                        } else if (item.template.type == 22) {

                        } else {
                            text = "Hành trang không còn chỗ trống";
                            Service.gI().sendThongBao(player, text);
                        }
            ...
            e.printStackTrace();
            System.err.println("Lỗi ở class Zone : pick item");
        }
        ...
        try {
            if (player.zone != null) {
                if (MapService.gI().isMapOffline(this.map.mapId)) {
                    if (player.isPet && player.isNewPet && this.equals(((Pet) player).master.zone)) {
...
//            }
        ...
        if ((((plInfo.isBoss || plInfo.isPet) || plInfo.isNewPet) || plInfo instanceof BossDHVT)
                || plInfo instanceof Referee) {
        } else {
...
//                msg.writer().writeBoolean(false);
                msg.writer().writeBoolean(mob.tempId == 77 ? true : false);
            }
