...
public class MobEffectSkill {
    ...
    public long lastTimeMaPhong3;
    public int timeMaPhong3;
    public boolean isMaPhong3;
    ...
    public void update() {
        ...
        if (isMaPhong3 && (Util.canDoWithTime(lastTimeMaPhong3, timeMaPhong3) || mob.isDie())) {
            removeMaPhong3();
        }
    }
    ...
    public void removeMaPhong3() {
        Message message;
        try {
            message = new Message(-112);
            message.writeByte(0);
            message.writeByte(getIndex());

            Service.getInstance().sendMessAllPlayerInMap(mob.zone, message);
            message.cleanup();
        } catch (Exception e) {
            e.printStackTrace();
        }
    ...
    public int getIndex() {
        for (int i = 0; i < mob.zone.mobs.size(); i++) {
            if (mob.zone.mobs.get(i) == mob) {
                return i;
            }
        }
        return -1;
