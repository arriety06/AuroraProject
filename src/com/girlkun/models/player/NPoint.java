...
public class NPoint {
    ...
    public boolean isHoaBang;
    public boolean CaiTrangHoaBang;

    public boolean isAnThan;
    public boolean CaiTrangAnThan;
    /**
     * Chỉ số cộng thêm
     */
    ...
    private void setPointWhenWearClothes() {
        ...
        List<Card> cards = player.Cards.stream().filter(r -> r != null && r.Used == 1).collect(Collectors.toList());
        if (cards != null) {
            for (Card card : cards) {
    ...
    private void setDameTrainArmor() {
        if (!this.player.isPet && !this.player.isBoss && !this.player.isNewPet) {
    ...
    private void setHpMax() {
        ...
        if (this.player.rewardBlackBall.timeOutOfDateReward[0] > System.currentTimeMillis()) {
            tlmau += RewardBlackBall.R1S;
        }
        this.hpMax += ((long) this.hpMax * tlmau / 100);
        //set nappa
...
//        }
    ...
    private void setMpMax() {
        ...
        if (this.player.itemTime != null && this.player.itemTime.isUseBoKhi) {
            this.mpMax += ((long) this.mpg * 1);
        }
        if (this.player.itemTime != null && this.player.itemTime.isUseBoKhi2) {
            this.mpMax += ((long) this.mpg * 1.2);
        }
        //ngọc rồng đen 3 sao
        if (this.player.rewardBlackBall.timeOutOfDateReward[2] > System.currentTimeMillis()) {
            tlki += RewardBlackBall.R3S;
        }
        //BTc2
        if (this.player.fusion.typeFusion == ConstPlayer.HOP_THE_PORATA2) {
            tlki += 15;
        ...
        this.mpMax += ((long) this.mpMax * tlki / 100);
...
//        }
    ...
    private void setDame() {
        ...
        if (this.player.itemTime != null && this.player.itemTime.isUseCuongNo) {
            this.dame += ((long) this.dameg * 1);
        }
        if (this.player.itemTime != null && this.player.itemTime.isUseCuongNo2) {
            this.dame += ((long) this.dameg * 1.2);
        }
        //ngọc rồng đen 1 sao
        if (this.player.rewardBlackBall.timeOutOfDateReward[0] > System.currentTimeMillis()) {
            tldame += RewardBlackBall.R1S;
        }
        //btc2
        if (this.player.fusion.typeFusion == ConstPlayer.HOP_THE_PORATA2) {
            tldame += 15;
        ...
        this.dame += ((long) this.dame * tldame / 100);
        //thức ăn
    ...
    private void setDef() {
        ...
        if (this.player.rewardBlackBall.timeOutOfDateReward[1] > System.currentTimeMillis()) {
            this.def += ((long) this.def * RewardBlackBall.R2S / 100);
        }
    }
    ...
    private void resetPoint() {
        ...
        this.tlHpGiamODo = 0;
        this.test = 0;
        this.multicationChuong = 0;
        this.IsBiHoaDa = false;
        this.CoCaiTrangHoaDa = false;
        this.suphu = 0;
        this.isHoaBang = false;
        this.isAnThan = false;
        this.CaiTrangAnThan = false;
        this.CaiTrangHoaBang = false;
        this.teleport = false;
        this.tlGold = 0;
    ...
    public int getDameAttack(boolean isAttackMob) {
        ...
        switch (skillSelect.template.id) {
            ...
            case Skill.DICH_CHUYEN_TUC_THOI:
                dameAttack *= 2;
                dameAttack = Util.nextInt((int) (dameAttack - ((long) dameAttack * 5 / 100)),
                        (int) (dameAttack + ((long) dameAttack * 5 / 100)));
        ...
        if (percentDameSkill != 0) {
            dameAttack = ((long) dameAttack * percentDameSkill / 100);
        }
...
//        }
        ...
        dameAttack = Math.abs(dameAttack);
        dameAttack = Util.nextInt((int) (dameAttack - ((long) dameAttack * 5 / 100)), (int) (dameAttack + ((long) dameAttack * 5 / 100)));
        if (player.isPl()) {
...
//            Logger.log(Logger.PURPLE, "Player: " + player.name + " Id: " + player.id + " Skill: " + player.playerSkill.skillSelect.template.name + "Dame: " + Util.numberToMoney(dameAttack) + " \n");
        ...
        if (power < getPowerLimit()) {
            ...
            if (this.player.isPet) {
                ...
                if (((Pet) this.player).master.nPoint.suphu > 0) {
                    tlexp += ((Pet) this.player).master.nPoint.suphu;
                }
            }
            tiemNang += ((long) tiemNang * tlexp / 100);
    ...
    public int subDameInjureWithDeff(int dame) {
        ...
        if (this.player.buffvtpt) {
            dame -= ((long) dame * 20 / 100);
        }
        if (dame < 0) {
            dame = 1;
        }
