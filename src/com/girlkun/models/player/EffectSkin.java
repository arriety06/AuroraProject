...
public class EffectSkin {
    ...
    private static final int TIME_HOA_DA = 5000;
    private static final int TIME_HOA_DA_PER_SECOND = 20000;

    public long TimeHoaBang;
    public long TimeBiHoaBang;
    private static final int TIME_HOA_BANG = 5000;

    public long Last_time_an_than;
    ...
    public void update() {
        ...
        // 
        if (this.player != null && this.player.nPoint != null && this.player.nPoint.IsBiHoaDa && (System.currentTimeMillis() - lastTimeBiHoaDa > TIME_HOA_DA)) {
        ...
        if (this.player != null && this.player.nPoint != null && this.player.nPoint.isHoaBang && (System.currentTimeMillis() - TimeBiHoaBang > TIME_HOA_BANG)) {
            this.player.nPoint.isHoaBang = false;
            Service.gI().SendMsgUpdateHoaDa(this.player, (byte) 0, (byte) 0, (byte) 42);
            Service.gI().Send_Caitrang(this.player);
            ItemTimeService.gI().removeItemTime(this.player, 11085);
        }
    }

    private void updateAnThan() {
        try {
            boolean CaiTrangAnThan = this.player.nPoint.CaiTrangAnThan;
            if (CaiTrangAnThan) {
                if (Util.canDoWithTime(Last_time_an_than, 10000)) {
                    this.player.nPoint.isAnThan = true;
                    Service.gI().SendRemovePlayer(this.player);
                    Last_time_an_than = System.currentTimeMillis();
                }
            }
            if (this.player.nPoint.isAnThan && Util.canDoWithTime(Last_time_an_than, 5000)) {
                this.player.nPoint.isAnThan = false;
                this.player.zone.load_Me_To_Another(this.player);
            }
    ...
    private void updateHoaDa() {
        try {
            if (this.player != null && this.player.zone != null && this.player.nPoint != null && !this.player.isDie()) {
                ...
                if (CoCaiTrangHoaDa) {
                    ...
                    if (Check && !MapService.gI().isMapOffline(player.zone.map.mapId)) { // thời gian hóa đá mỗi lần
                        List<Player> players = new ArrayList<>();
                        List<Player> playersMap = this.player.zone.getNotBosses();
                        if (!playersMap.isEmpty()) {
                            for (int i = 0; i < playersMap.size(); i++) {
                                Player pl = playersMap.get(i);
                                if (pl != null && !this.player.equals(pl) && pl.zone != null && pl.zone.equals(player) && !pl.isReferee && !pl.isNewPet && !pl.isBoss && !pl.isDie()
    ...
    private void updateHoaBang() {
        try {
            if (this.player != null && this.player.zone != null && this.player.nPoint != null && !this.player.isDie()) {
                boolean CaiTrangHoaBang = this.player.nPoint.CaiTrangHoaBang;
                if (CaiTrangHoaBang) {
                    boolean Check = System.currentTimeMillis() - TimeHoaBang > TIME_HOA_DA_PER_SECOND;
                    if (Check && !MapService.gI().isMapOffline(player.zone.map.mapId)) { // thời gian hóa đá mỗi lần
                        List<Player> players = new ArrayList<>();
                        List<Player> playersMap = this.player.zone.getNotBosses();
                        if (!playersMap.isEmpty()) {
                            for (int i = 0; i < playersMap.size(); i++) {
                                Player pl = playersMap.get(i);
                                if (pl != null && !this.player.equals(pl) && pl.zone != null && pl.zone.equals(player) && !pl.isReferee && !pl.isNewPet && !pl.isBoss && !pl.isDie()
                                        && Util.getDistance(this.player, pl) <= 200 && !pl.nPoint.isHoaBang) {
                                    players.add(pl);
                                }
                            }
                        }
                        if (!players.isEmpty()) {
                            for (int i = 0; i < players.size(); i++) {
                                ...
                                if (pl != null && !this.player.equals(pl) && pl.zone != null && pl.zone.equals(player) && !pl.isReferee && !pl.isNewPet && !pl.isBoss && !pl.isDie()
                                        && Util.getDistance(this.player, pl) <= 200 && !pl.nPoint.isHoaBang) {
                                    pl.nPoint.isHoaBang = true;
                                    Service.gI().SendMsgUpdateHoaDa(pl, (byte) 1, (byte) 0, (byte) 42);
                                    Service.gI().Send_Caitrang(pl);
                                    ItemTimeService.gI().sendItemTime(pl, 11085, (int) TIME_HOA_BANG / 1000);
                                    Service.gI().chat(pl, textHoaDa[Util.nextInt(0, textHoaDa.length - 1)]);
                                    if (pl.effectSkin != null) {
                                        pl.effectSkin.TimeBiHoaBang = System.currentTimeMillis();
                                    }
                                }
                            }
                        }
                        this.TimeHoaBang = System.currentTimeMillis();
                        if (this.player.effectSkin != null) {
                            Service.gI().chat(this.player, "Ẹc Ẹc...");
                        }
                    }
                }
    ...
    private void updateCTHaiTac() {
        if (this.player != null && this.player.setClothes.ctHaiTac != -1
    ...
    private void updateXenHutXungQuanh() {
        try {
            ...
            if (param > 0 && !MapService.gI().isMapOffline(player.zone.map.mapId)) {
                if (this.player != null && this.player.zone != null && !this.player.isDie() && Util.canDoWithTime(lastTimeXenHutHpKi, 5000)) {
                    int hpHut = 0;
                    int mpHut = 0;
                    List<Player> players = new ArrayList<>();
                    List<Player> playersMap = this.player.zone.getNotBosses();
                    for (Player pl : playersMap) {
                        if (pl != null && pl.zone != null && pl.zone.equals(player) && !this.player.equals(pl) && !pl.isBoss && !pl.isDie()
                    ...
                    for (Mob mob : this.player.zone.mobs) {
                        if (mob != null && mob.point != null && mob.point.gethp() > 1) {
                    ...
                    for (Player pl : players) {
                        if (pl != null && pl.zone != null && pl.zone.equals(player) && player.nPoint != null && !this.player.equals(pl) && !pl.isBoss && !pl.isDie()
                                && Util.getDistance(this.player, pl) <= 200) {
    ...
    private void updateOdo() {
        try {
            ...
            if (param > 0 && !MapService.gI().isMapOffline(player.zone.map.mapId)) {
                if (this.player != null && this.player.zone != null && !this.player.isDie() && Util.canDoWithTime(lastTimeOdo, 10000)) {
                    List<Player> players = new ArrayList<>();
    ...
    private void updateTrainArmor() {
        try {
            if (this.player != null && this.player.nPoint != null && this.player.inventory != null) {
