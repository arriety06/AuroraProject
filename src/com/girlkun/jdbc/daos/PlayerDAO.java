...
public class PlayerDAO {
    ...
    public static boolean createNewPlayer(int userId, String name, byte gender, int hair) {
        try {
            ...
            dataArray.add(0);
            dataArray.add(ServerManager.gI().getNumPlayer() + 1);
            dataArray.add(gender == 0 ? 200 : 100); //hp max
            dataArray.add(gender == 1 ? 200 : 100); //ki max
            dataArray.add(gender == 2 ? 15 : 10); //sức đánh max
            dataArray.add(0);//giáp max
...
//            dataArray.add(ServerManager.gI().getNumPlayer() + 1);
//            dataArray.add(0);
//            String sieuhang = dataArray.toJSONString();
            GirlkunDB.executeUpdate("insert into player"
                    ...
                    + "data_task, data_mabu_egg, data_egg_linhthu, data_charm, skills, skills_shortcut, pet,"
                    + "data_black_ball, data_side_task, data_item_time_sieu_cap, data_may_do, data_card, info_achievement) "
                    ...
                    itemTime, task, mabuEgg, EggLinhThu, charms, skills, skillsShortcut, petData, dataBlackBall, dataSideTask, itemTimeSC, itemMayDo, data_card, info_achive);
    ...
    public static void updatePlayer(Player player) {
        if (player != null && player.iDMark != null && player.iDMark.isLoadedAllDataPlayer()) {
            ...
            try {
                ...
                //data item time maydo
                dataArray.add((player.itemTime.isUseMayDoCua ? (ItemTime.TIME_MAY_DO_CA - (System.currentTimeMillis() - player.itemTime.TimeMayDoCua)) : 0));
                dataArray.add((player.itemTime.isUseMayDoTom ? (ItemTime.TIME_MAY_DO_TOM - (System.currentTimeMillis() - player.itemTime.TimeMayDoTom)) : 0));
                dataArray.add((player.itemTime.isUseMayDoCa ? (ItemTime.TIME_MAY_DO_CA - (System.currentTimeMillis() - player.itemTime.TimeMayDoCa)) : 0));
                dataArray.add((player.itemTime.isUseMayDoOc ? (ItemTime.TIME_MAY_DO_OC - (System.currentTimeMillis() - player.itemTime.TimeMayDoOc)) : 0));
                String itemMayDo = dataArray.toJSONString();
                dataArray.clear();
...
//                String sieuhang = dataArray.toJSONString();
//                dataArray.clear();
                String query = " update player set info_achievement=?,data_card=?, data_may_do =?, data_item_time_sieu_cap = ?,head = ?, have_tennis_space_ship = ?,"
...
//                Logger.success("Total time save player " + player.name + " thành công! " + (System.currentTimeMillis() - st) + "\n");
    ...
    public static boolean subGoldBar(Player player, int num) {
        ...
        try (Connection con = GirlkunDB.getConnection();) {
            ...
            ps.setInt(1, num);
            ps.setInt(2, player.getSession().getIsActive() ? 1 : 0);
            ps.setInt(3, player.getSession().getUserId());
            ps.executeUpdate();
            player.getSession().setGoldBar(player.getSession().getGoldBar() - num);
        } catch (Exception e) {
            e.printStackTrace();
    ...
    public static boolean subVnd(Player player, int num) {
        ...
        try (Connection con = GirlkunDB.getConnection();) {
            if (!player.getSession().getIsActive()) {
                player.getSession().setIsActive(true);
            }
            ...
            ps.setInt(3, player.getSession().getUserId());
            ps.executeUpdate();
            player.getSession().setVnd(player.getSession().getVnd() - num);
        } catch (Exception e) {
            e.printStackTrace();
    ...
    public static boolean subPointshare(Player player, int num) {
        PreparedStatement ps = null;
        try (Connection con = GirlkunDB.getConnection();) {
            ps = con.prepareStatement("update account set diemgioithieu = (diemgioithieu - ?) where id = ?");
            ps.setInt(1, num);
            ps.setInt(2, player.getSession().getUserId());
            ps.executeUpdate();
            player.getSession().setPointshare(player.getSession().getPointshare() - num);
        } catch (Exception e) {
            e.printStackTrace();
            Logger.logException(PlayerDAO.class, e, "Lỗi update point share" + player.name);
            return false;
        } finally {
            try {
                ps.close();
    ...
    public static boolean setIs_gift_box(Player player) {
        ...
        try (Connection con = GirlkunDB.getConnection();) {
            ...
            ps.setInt(1, player.getSession().getUserId());
            ps.executeUpdate();
            ps.close();
        } catch (Exception e) {
            e.printStackTrace();
    ...
    public static void updateItemReward(Player player) {
        ...
        for (Item item : player.getSession().getItemsReward()) {
            if (item.isNotNullItem()) {
